...
// ---------------------------------------------------------------------------
// SPECTRA AudioWorklet processor — runs the SoundTouch WSOLA pipeline on the
// audio rendering thread so pitch/tempo processing never janks the UI thread
// (and vice versa). The classes above are the vendored soundtouchjs DSP with
// the module export stripped so this file loads in every AudioWorklet
// implementation (Firefox does not support imports in worklet modules).
// ---------------------------------------------------------------------------
...
class SoundTouchProcessor extends AudioWorkletProcessor {
    ...
    this.port.onmessage = (e) => {
      ...
      switch (m.type) {
        case 'load':
          ...
          this.source = new WorkletBufferSource(m.left, m.right);
        ...
        case 'params':
          if (this.st) {
            if (m.keyLock) {
              this.st.tempo = m.tempo;
              this.st.rate = 1;
              this.st.pitchSemitones = m.semis;
            } else {
              this.st.tempo = 1;
              this.st.rate = m.tempo;
              this.st.pitchSemitones = m.semis;
            }
          }
          break;
        case 'seek':
          if (this.filter) {
            this.filter.sourcePosition = Math.max(0, Math.floor(m.frames));
            this.ended = false;
          }
          break;
  ...
  process(inputs, outputs) {
    const out = outputs[0];
    if (!this.playing || !this.filter || out.length === 0) return true;
    ...
    if (currentTime - this._lastReport > 0.1) {
      this._lastReport = currentTime;
      this.port.postMessage({ type: 'pos', sourcePosition: this.filter.sourcePosition });
    }
    return true;
  }
}
